...
- Understand the role of the **management account** and **Organizational Units (OUs)**  
...
Creates an AWS Organization and an Organizational Unit (OU).  
...
Creates and attaches an SCP that denies EC2 actions unless IMDSv2 is required.  
The SCP is attached to an OU or Root (OU is recommended for safe rollout).
...
In this lab, SCPs are applied to an OU — not directly to the management account.
...
- **Detective:** detect issues after they happen (logs, alerts, scanners)

---

## Architecture (High-Level)

![AWS Organizations SCP – IMDSv2 Enforcement Scope](docs/diagrams/aws-org-scp-imdsv2-architecture.png.png)
...
- Terraform will create an OU for safe policy attachment  
...
1. Start with **Phase 1** to bootstrap the Organization foundation  
2. Proceed to **Phase 2** to implement and validate the preventive SCP  
3. Use CloudTrail to collect enforcement evidence  

Each phase contains a dedicated README with step-by-step instructions.

---

## Safety Notice

SCPs are **organization-level guardrails** and can block actions across many accounts.  
Always:

- Test in non-production accounts first  
- Review policy logic carefully  
- Roll out gradually  
...
Review and adapt all security controls to your organization’s risk profile and compliance requirements before production use.
